// =====================================================================
// jiaminglake.js
// All application logic for 2026 戒茂斯上嘉明湖四日縱走
...
// 所有會改到雲端 waypoint.json 的寫入類 API，一律排隊依序送出，不要同時並發。
// 伺服器那邊雖然有 compare-and-swap + retry，但重試次數有上限；操作太快時
// （例如連續按上一張/下一張排序、或連續編輯不同照片說明）同時打出一堆並發請求，
// 會互相搶著改同一份檔案，容易把重試次數用光而默默失敗，畫面上看起來像是
// 「後面的修改沒存到、資料錯亂」。改成排隊送出後，請求永遠照使用者操作的順序
// 一個一個真的送到 worker，不會互搶。
var apiWriteQueue = Promise.resolve();
function queuedFetch(url, opts){
  var result = apiWriteQueue.then(function(){
    return fetch(url, opts);
  });
  // 就算這次失敗也要讓佇列繼續往下走，不要卡住後面排隊的請求
  apiWriteQueue = result.then(function(){}, function(){});
  return result;
}
